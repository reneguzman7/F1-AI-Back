...
def get_prediction(input_data: InputData):
    try:
        # Intentamos hacer la predicci�n con los datos
        prediction = predict_podium(input_data)
        return {"podio": prediction}
    
    except ValidationError as e:
        # En caso de error de validaci�n, capturamos los campos incorrectos
        errors = []
...
def send_data():
    ...
    try:
        ...
        for piloto in data:
            try:
                # Si ya hay 3 podios, el resto ser� autom�ticamente "Fuera de podio"
                if podios_contador >= 3:
                    resultados.append({"Piloto": piloto["Piloto"], "Resultado": "Fuera de podio"})
                    continue

                # Hacer la predicci�n directamente
